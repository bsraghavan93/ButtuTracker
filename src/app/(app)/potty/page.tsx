...
export default function PottyPage() {
  const { baby } = useBaby();
  const { pottyLogs, loading, refresh } = useLogs(baby?.id, 14);
  ...
  const trend: TrendPoint[] = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      const count = pottyLogs.filter((p) => new Date(p.occurred_at) >= dayStart && new Date(p.occurred_at) < dayEnd).length;
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("potty_logs").delete().eq("id", id);
    refresh();
  }
  ...
  return (
    <div className="flex flex-col gap-5 pt-2">
      <h1 className="text-xl font-semibold">Potty</h1>

      <div>
        <p className="mb-2 text-sm font-medium text-foreground/70">Potty visits per day, last 7 days</p>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {!loading && pottyLogs.length === 0 && (
            <p className="text-sm text-foreground/50">No potty logs yet — tap the + button to add one.</p>
          )}
        </div>
      </div>
    </div>
